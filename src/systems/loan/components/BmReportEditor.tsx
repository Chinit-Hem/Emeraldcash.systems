...
export default function BmReportEditor({ value, onChange, readOnly, isKhmer, reportDate, branch, reporterName }: Props) {
  ...
  // BM needs the complete overview while entering a manual report. The compact
  // version is only for reviewers, where empty source counters add no value.
  const showSourceMetrics = !isManualReport || !readOnly;
  ...
  return <section className="space-y-5 p-3 text-slate-900 dark:text-slate-100 sm:p-5">
    ...
    <div className={section === "overview" ? "space-y-5" : "hidden print:block"}>
      <div className={`grid grid-cols-2 gap-3 ${showSourceMetrics ? "lg:grid-cols-4" : "lg:grid-cols-2"}`}>
        {showSourceMetrics ? <><Metric label={text("LS reports", "របាយការណ៍ LS")} value={daily.lsReports} helper={text("Today", "ថ្ងៃនេះ")} color="border-blue-200 bg-blue-50 text-blue-900 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-200" />
