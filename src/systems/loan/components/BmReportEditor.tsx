...
export default function BmReportEditor({ value, onChange, readOnly, isKhmer, reportDate, branch, reporterName }: Props) {
  ...
  const generatedSourceReadOnly = readOnly || value.mode === "generated";
...
<div className={section === "kpis" ? "grid gap-4 lg:grid-cols-2" : "hidden print:grid print:gap-4"}>{BM_KPIS.map(([id, km, en, percent]) => { const row = kpis.find((item) => item.id === id) || { id, target: "", daily: "", monthly: "", note: "" }; const computed = bmKpiValues(kpis, row);
      ...
      return <article key={id} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
        ...
        {readOnly ? null : <>
          ...
          <div className={`mt-3 grid gap-3 sm:grid-cols-3 ${open ? "" : "hidden print:grid print:gap-3"}`}><NumberField label={text("Target", "គោលដៅ") + (percent ? " (%)" : "")} value={row.target} onChange={(next) => update("target", next)} readOnly={generatedSourceReadOnly} /><NumberField label={text("Today", "ថ្ងៃនេះ")} value={displayDaily} onChange={(next) => update("daily", next)} readOnly={generatedSourceReadOnly} /><NumberField label={text("This month", "ខែនេះ")} value={displayMonthly} onChange={(next) => update("monthly", next)} readOnly={generatedSourceReadOnly} /><div className="sm:col-span-3"><TextField label={text("Note", "កំណត់សម្គាល់")} value={row.note} onChange={(next) => update("note", next)} readOnly={readOnly} /></div></div>
        </>}
      </article>; })}</div>
    ...
    <div className={section === "team" ? "space-y-6" : "hidden print:block"}>
      ...
      <div>
        ...
        <div className="space-y-4">{value.staff.map((row, index) => { const open = Boolean(staffOpen[index]); const approved = Number(row.approved) || 0; const collected = Number(row.collected) || 0; const rejected = Number(row.rejected) || 0; const contacts = Number(row.contacts) || 0; const rate = approved + rejected + collected > 0 ? (collected / (approved + rejected + collected)) * 100 : null; return <article key={index} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-3"><h4 className="min-w-0 truncate font-bold">{row.name || `${text("Loan specialist", "អ្នកឯកទេសផ្ដល់កម្ចី")} ${index + 1}`}</h4><div className="flex shrink-0 items-center gap-2">{!generatedSourceReadOnly ? <EditToggle open={open} onClick={() => toggle(setStaffOpen, index)} label={text("Edit", "កែ")} labelOpen={text("Done", "រួចរាល់")} /> : null}{!generatedSourceReadOnly ? <button type="button" aria-label={text("Remove", "លុប")} onClick={() => onChange({ ...value, staff: value.staff.filter((_, rowIndex) => rowIndex !== index) })} className="flex h-10 w-10 items-center justify-center rounded-lg text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button> : null}</div></div>
      ...
      <div>
        ...
        <div className="space-y-4">{value.accounts.map((row, index) => { const open = Boolean(accountOpen[index]); return <article key={index} className="rounded-xl border border-violet-200 bg-violet-50/40 p-4 dark:border-violet-900 dark:bg-violet-950/20">
          ...
          <div className={`mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 ${open || generatedSourceReadOnly ? "" : "hidden print:grid print:gap-3"}`}><div className="sm:col-span-2 lg:col-span-3"><TextField label={text("Name", "ឈ្មោះ")} value={row.name} onChange={(next) => updateAccount(index, "name", next)} readOnly={generatedSourceReadOnly} /></div>{(["due", "paid", "dueAmount", "paidAmount"] as const).map((key) => <NumberField key={key} label={text(({ due: "Due customers", paid: "Paid customers", dueAmount: "Due amount", paidAmount: "Paid amount" } as const)[key], ({ due: "អតិថិជនត្រូវបង់", paid: "អតិថិជនបានបង់", dueAmount: "ប្រាក់ត្រូវបង់", paidAmount: "ប្រាក់បានបង់" } as const)[key])} value={row[key]} onChange={(next) => updateAccount(index, key, next)} readOnly={generatedSourceReadOnly} />)}</div>
        </article>; })}
        {!generatedSourceReadOnly ? <button type="button" onClick={() => onChange({ ...value, accounts: [...value.accounts, emptyBmAccountRow()] })} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-lg border border-violet-300 px-4 font-semibold text-violet-700 hover:bg-violet-50"><Plus className="h-4 w-4" />{text("Add account staff", "បន្ថែមបុគ្គលិកគណនេយ្យ")}</button> : null}
        </div>
      </div>
    </div>
