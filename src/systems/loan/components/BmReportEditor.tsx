...
<div className={section === "kpis" ? "grid gap-4 xl:grid-cols-2" : "hidden print:grid print:gap-4"}>{BM_KPIS.map(([id, km, en, percent]) => { const row = kpis.find((item) => item.id === id) || { id, target: "", daily: "", monthly: "", note: "" }; const computed = bmKpiValues(kpis, row);
    ...
    <div className={section === "team" ? "space-y-6" : "hidden print:block"}>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-5 dark:border-slate-800">
          ...
          <div className="flex flex-wrap items-center gap-2"><div className="inline-flex rounded-lg border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-950"><button type="button" onClick={() => setTeamView("cards")} className={`min-h-8 rounded-md px-3 text-xs font-semibold ${teamView === "cards" ? "bg-white text-slate-900 shadow-sm dark:bg-slate-800 dark:text-white" : "text-slate-500"}`}>{text("Cards", "កាត")}</button><button type="button" onClick={() => setTeamView("list")} className={`min-h-8 rounded-md px-3 text-xs font-semibold ${teamView === "list" ? "bg-white text-slate-900 shadow-sm dark:bg-slate-800 dark:text-white" : "text-slate-500"}`}>{text("List", "បញ្ជី")}</button></div>{!generatedSourceReadOnly ? <button type="button" onClick={() => onChange({ ...value, staff: [...value.staff, emptyBmStaffRow()] })} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-emerald-600 px-3.5 text-sm font-semibold text-white hover:bg-emerald-700"><Plus className="h-4 w-4" />{text("Add staff", "បន្ថែមបុគ្គលិក")}</button> : null}</div>
        </div>
        <div className={`${teamView === "cards" ? "grid gap-3 sm:grid-cols-2 xl:grid-cols-3" : "space-y-2"} p-3 sm:p-4`}>{value.staff.map((row, index) => <article key={index} className={`rounded-xl border border-slate-200 bg-slate-50/60 shadow-sm dark:border-slate-700 dark:bg-slate-950/40 ${teamView === "cards" ? "p-4 sm:p-5" : "p-3 sm:flex sm:items-center sm:gap-4"}`}>
    ...
    <div className={section === "periods" ? "grid gap-4 xl:grid-cols-3" : "hidden print:grid print:gap-4 xl:print:grid-cols-3"}>
