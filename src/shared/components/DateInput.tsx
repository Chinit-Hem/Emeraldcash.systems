...
type DateInputProps = {
  ...
  max?: string;
  placeholder?: string;
  /** Optional per-day status dots, used by report calendars to show source data. */
...
export function DateInput({ value, onChange, type = "date", className = "", id, name, title, disabled, required, min, max, placeholder, dateMarkers, "aria-label": ariaLabel }: DateInputProps) {
        ...
        <div ref={calendarRef} id={`${inputId}-calendar`} role="dialog" aria-label={title || "Date picker"} data-date-input-calendar="true" data-placement={openAbove ? "top" : "bottom"} style={calendarStyle} className="max-w-[calc(100vw-1rem)] overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-2xl dark:border-slate-700 dark:bg-slate-950">
          ...
          {type === "month" ? (
            ...
            <>
              ...
              <div>{weeks.map((week, weekIndex) => <div key={weekIndex} className="grid h-9 grid-cols-8 border-b border-slate-200 last:border-b-0 dark:border-slate-800"><span className="flex items-center justify-center text-xs font-semibold text-slate-400">{weekNumber(week[0])}</span>{week.map((date) => { const iso = isoDate(date); const currentMonth = date.getMonth() === month.getMonth(); const selected = isoDate(draftDate) === iso; const today = isoDate(new Date()) === iso; const marker = dateMarkers?.[iso]; const unavailable = Boolean((min && iso < min.slice(0, 10)) || (max && iso > max.slice(0, 10))); return <button key={date.toISOString()} type="button" disabled={unavailable} onClick={() => selectDate(date)} title={marker === "ready" ? "Source data ready" : marker === "pending" ? "Source data pending review" : undefined} className={`relative m-0.5 flex items-center justify-center rounded-full text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-30 ${selected ? "bg-emerald-600 text-white" : currentMonth ? `text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 dark:text-slate-200 dark:hover:bg-emerald-500/10 dark:hover:text-emerald-300 ${today ? "ring-1 ring-emerald-500" : ""}` : "text-slate-400 hover:bg-slate-100 dark:text-slate-600 dark:hover:bg-slate-800"}`}>{date.getDate()}{marker ? <span aria-hidden="true" className={`absolute bottom-0 h-2 w-2 rounded-full ring-1 ring-white dark:ring-slate-950 ${selected ? "bg-white" : marker === "ready" ? "bg-emerald-500" : "bg-amber-500"}`} /> : null}</button>; })}</div>)}</div>
