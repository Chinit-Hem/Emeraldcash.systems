...
export default function TopBar({
  ...
  // The system switcher is global navigation. Keep every permitted system visible
  // even while a user is working inside one workspace (for example Loan Management).
      ...
      <div className="flex h-full w-full items-center gap-3 pl-[max(0.5rem,env(safe-area-inset-left))] pr-[max(0.5rem,env(safe-area-inset-right))] sm:px-5">
          ...
          <div className="ml-auto flex min-w-0 shrink-0 items-center justify-end gap-1 sm:gap-2">
            ...
            <div className="relative hidden lg:block">
              ...
              <FloatingTopBarMenu open={isDateMenuOpen} anchorRef={dateButtonRef} menuRef={dateMenuRef}><div role="dialog" aria-label={language === "km" ? "កាលបរិច្ឆេទការងារ" : "Working date"} className="w-64 rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl shadow-slate-900/15 dark:border-slate-700 dark:bg-slate-900 dark:shadow-black/40"><DateInput title={language === "km" ? "កាលបរិច្ឆេទការងារ" : "Working date"} value={workingDate} onChange={setWorkingDate} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100" /><button type="button" onClick={() => { const now = new Date(); const parts = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Phnom_Penh", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(now); const year = parts.find((part) => part.type === "year")?.value; const month = parts.find((part) => part.type === "month")?.value; const day = parts.find((part) => part.type === "day")?.value; if (year && month && day) setWorkingDate(`${year}-${month}-${day}`); }} className="mt-2 min-h-9 px-1 text-sm font-semibold text-emerald-700 hover:text-emerald-800 dark:text-emerald-300">{language === "km" ? "ប្រើថ្ងៃនេះ" : "Use today"}</button></div></FloatingTopBarMenu>
            </div>
